...
#define ____cacheline_aligned2 __attribute__((aligned(CACHE_LINE_PREFETCH_UNIT * L1_CACHE_BYTES)))
...
static inline void
clwb(volatile void* p) {
    asm volatile(".byte 0x66; xsaveopt %0" : "+m"(p));
}

#ifndef __KERNEL__
static inline void
cpuid(int i, unsigned int* a, unsigned int* b, unsigned int* c, unsigned int* d) {
    /* https://bit.ly/2uGziVO */
    __asm __volatile("cpuid" : "=a"(*a), "=b"(*b), "=c"(*c), "=d"(*d) : "a"(i), "c"(0));
}
#endif
